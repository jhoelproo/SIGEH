...
class SQLiteWriteCoordinator:
    ...
    def acquire(self, operation: str, timeout: float = SQLITE_WRITE_TIMEOUT_SECONDS) -> None:
        ...
        with self._condition:
            ...
            while self._owner_thread_id is not None or self._waiters[0] is not token:
                ...
                if remaining <= 0:
                    ...
                    LOGGER.error(
                        ...
                        "connection_id=%s writer_queue_depth=%s lock_holder_operation=%s "
                        "lock_holder_thread=%s lock_holder_hold_ms=%.1f",
                        operation,
                        threading.current_thread().name,
                        elapsed_ms,
                        id(self),
            ...
            LOGGER.info(
                "SQLITE_WRITE_ACQUIRED operation=%s thread=%s wait_ms=%.1f queue_depth=%s",
                self._owner_operation,
                self._owner_thread_name,
                self._owner_wait_ms,
                len(self._waiters),
    ...
    def release(self) -> None:
        ...
        with self._condition:
            ...
            LOGGER.info(
                "SQLITE_WRITE_RELEASED operation=%s thread=%s wait_ms=%.1f hold_ms=%.1f queue_depth=%s",
                self._owner_operation,
                self._owner_thread_name,
                self._owner_wait_ms,
                hold_ms,
...
class CoordinatedSQLiteConnection:
    ...
    def __init__(self, database: str | Path, *, operation: str = "sqlite-write"):
        self.database = assert_private_local_database(database)
    ...
    def _ensure_write_lock(self, operation: str | None = None) -> None:
        if self._write_locked:
            return
        self.coordinator.acquire(operation or self.operation)
...
def connect_local_sqlite(
    ...
    *,
    ...
    return CoordinatedSQLiteConnection(database, operation=operation)
